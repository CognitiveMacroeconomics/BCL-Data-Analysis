...
LVTSBCLs.dir <- "C:/Projects/RawData/BOC_LVTS_BCL_Clean";
...
mergedLVTSBCLsNameString <- c("C:/Projects/LVTSBCLDataTables/YrsSpecificLVTSBCLBOCCleanYM2014-14DS.Rdata");
...
for (i in year.dir){
  setwd(i);
  tmp <- list.files(pattern="20.[[:digit:]]{3,4}.txt");
...
lvts.file.process <- function(x){
  ...
  colnames(clean.data) <- c("date","time","grantor","grantee","sequence","credit_limit");
