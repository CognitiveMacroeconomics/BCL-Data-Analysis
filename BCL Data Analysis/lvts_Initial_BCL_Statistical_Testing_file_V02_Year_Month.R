...
startYear  <- 2006;
endYear <- 2014;
summaryGrantorFIStatsSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLSummaryStatisticsMonthlyByYear2006-2014.Rdata");
grantorFILVTSBCLTimeSeriesDataTableSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLTimeSeriesDataTableMonthlyByYear2006-2014.Rdata");
grantorFILVTSBCLTimeSeriesXLSXSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLTimeSeriesXLSXSeries2006-2014.xlsx");
grantorFILVTSBCLTotalXLSXSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLTotalXLSXSeries2006-2014.xlsx");
...
load("C:/Projects/LVTSBCLDataTables/YearsSpecificLVTSBCLYM2006-14DataSeries.Rdata");
gc();
...
getListOfFINames <- function(inputDataTable, colNames){
  ...
  } else if("grantees" %in% colNames){#otherwise check if the field reciever is in the list of column headers and if so get the unique list of grantor names
...
##The following method filters a summary statistical data table containing information about the payments amounts and further summarises
##this data by grantor-reciever FI pairing and year and month to get summary statistics for number (volume) of payments sent by the FI to all other FIs combined
...
if(!isDaily){#if analysis is not based on daily level granularity
  ...
  print(now());#used to record speed of computations
  grantorFIsBCLVolumeTimeSeries  <- getFIVolumeValueTimeSeries(grantorFILVTSBCLVolumeValueSummaryStatisticsMonthlyByYear, "ROYCCA", "volume",vectorColumnNames);
  grantorFIsBCLValueTimeSeries <- getFIVolumeValueTimeSeries(grantorFILVTSBCLVolumeValueSummaryStatisticsMonthlyByYear, "ROYCCA", "value",vectorColumnNames);
  gc();#free up some memory
}else{
  print("This file only handles monthly frequency analysis. For daily analysis, use lvts_Initial_Statistical_Testing_file_V03_Year_Month_Day.R file");
...
if(!isDaily){#if analysis is not based on daily level granularity
  ...
  print(now());#used to record speed of computations
  ##the following ggplot2 plot depicts the minimum and maximum mean amount sent by each FI to a recieving FI. The plots generated are sperated into seperate
