##This is class to process and summarise bank of canada cleaned BCL data
##
## Author : Segun Bewaji
## Modified : Segun Bewaji
## Modifications Made:
##        1: Reworking based on Bank of Canada cleaned BCL data. 
##           NOTE: 
##                   
##                 
##                  
...
fiBCLcDataLoadPath <- c("C:/Projects/LVTSBCLDataTables/YrsSpecificLVTSBCLBOCCleanYM2014-14DS.Rdata")
...
summaryGrantorFIStatsSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLSummaryStatisticsMonthlyByYear2014-2014.Rdata");
grantorFILVTSBCLTimeSeriesDataTableSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLTimeSeriesDataTableMonthlyByYear2014-2014.Rdata");
grantorFILVTSBCLTimeSeriesXLSXSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLTimeSeriesXLSXSeries2014-2014.xlsx");
grantorFILVTSBCLTotalXLSXSavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLTotalXLSXSeries2014-2014.xlsx");
grantorFILVTSBCLTimeSeriesSTATASavePath <- c("C:/Projects/FundingDataTables/grantorFILVTSBCLTimeSeriesSTATASeries2014-2014.dta");
...
##The monthlyDataExtraction method/function takes an input data.table/data frame and compresses it to a smaller
##version containing the data for the specific month and year of interest
getDailyDataRangeExtraction <- function(inputDataTable, yr, startMth, endMth, lookupColumnName, colNames){
  gc();
  temporaryTable <- NULL;
  if(endMth <= startMth){
    return(print("start month is greater than end month: please chakc and correct entry"))
...
getGrantorGranteeFIPairLVTSQuarterlySummaryStatistics <- function(inputDataTable, colNames){
  ...
  if("credit_limit" %in% colNames & "grantor" %in% colNames 
    ...
    temporarySummaryTable <- inputDataTable[ ,list(
      ...
      Mode_Credit_Limit = as.numeric(names(table(credit_limit))[which.max(table(credit_limit))]),
...
if(!isDaily){#if analysis is not based on daily level granularity
  ...
  monthlyBCLsValueGrantorRangeScatterPlot <- ggplot(grantorGranteeFITotalBCLMonthlySummaryStatistics, 
                                                    aes(x=(Mode_Credit_Limit/1000000), y=grantee, color=grantee)) +
  ...
  monthlyBCLsValueGrantorRangeScatterPlot <- monthlyBCLsValueGrantorRangeScatterPlot + 
    labs(x="Mean Daily Bilateral Credit Limit Granted Per Month (in millions)");
  gc();#free up momory
  
  ##grantor Specific charts
  
  grantorFILVTSPaymentsValueBoxPlot <- ggplot(grantorGranteeFITotalBCLMonthlySummaryStatistics,
                                              aes(x=grantor, y=(Mode_Credit_Limit/1000000), color=grantor)) +
    geom_boxplot() + scale_y_continuous(label=dollar);
  
  grantorFILVTSPaymentsValueBoxPlot <- grantorFILVTSPaymentsValueBoxPlot + 
    labs(y="Mean Daily Bilateral Credit Limit Granted Per Month (in millions)");
  
  grantorFILVTSMeanValueSeriesLinePlot <- ggplot(grantorGranteeFITotalBCLMonthlySummaryStatistics,
                                                 aes(x=Year_Month, y=(Mode_Credit_Limit/1000000), color=grantee)) +
    geom_line(aes(group=grantee, order = sample(seq_along(Year_Month))))+ scale_y_continuous(label=dollar) + facet_wrap(~ grantor);
  
  grantorFILVTSMeanValueSeriesLinePlot <- grantorFILVTSMeanValueSeriesLinePlot + 
    labs(y="Mean Daily Bilateral Credit Limit Granted Per Month (in millions)");
  grantorFILVTSMeanValueSeriesLinePlot <- grantorFILVTSMeanValueSeriesLinePlot + labs(x="Years and Months");
...
write.dta(grantorGranteeFITotalBCLQuarterlySummaryStatistics,file=c("C:/Projects/LVTSBCLDataTables/grantorGranteeFITotalBCLQuarterlySummaryStatisticsSTATASeries.dta"));
write.xlsx(grantorGranteeFITotalBCLQuarterlySummaryStatistics,file=c("C:/Projects/LVTSBCLDataTables/grantorGranteeFITotalBCLQuarterlySummaryStatisticsSTATASeries.xlsx"));
